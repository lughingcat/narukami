...
document.addEventListener('DOMContentLoaded', function(){
    ...
    var bannerLink = document.querySelector('.banner-content-wrap a');
    ...
    function dismissBanner() {
        localStorage.setItem("bannerDismissedTime", new Date().getTime());
        parentEl.classList.add('remove-banner');
        parentEl.style.opacity = "0";
        parentEl.style.pointerEvents = "none";
    }
    ...
    if (bannerDlBtn) {
        bannerDlBtn.addEventListener('click', function(e){
            e.preventDefault();
            dismissBanner();
        });
    }
